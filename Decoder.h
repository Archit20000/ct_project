...
{
public:
int **tanner,**tannerval,**varcon,n,k,maxconvar,maxconcheck,*rdecoded;
...
cout<<endl;
	...
	int decz(DigitalCommunication a,int choice)
	{
		...
		int i,j,kk,c,b,majority[1+maxconvar],s,rdecoded[n],passing[maxconvar+1][n],rreceived[n],check=0,tempm[1+maxconvar],check2=0;
		here:
		if(choice==0)
		{
		    for(i=0;i<n;i++)
		...
		for(check=0;check<100 && check2<100;check++)
		{
